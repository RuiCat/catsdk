...
func RectFromPoints(pts ...Point) Rect {
	...
	r := Rect{
		X: r1.Interval{Min: pts[0].X, Max: pts[0].X},
		Y: r1.Interval{Min: pts[0].Y, Max: pts[0].Y},
	}
...
func RectFromCenterSize(center, size Point) Rect {
	return Rect{
		r1.Interval{Min: center.X - size.X/2, Max: center.X + size.X/2},
		r1.Interval{Min: center.Y - size.Y/2, Max: center.Y + size.Y/2},
	}
}
