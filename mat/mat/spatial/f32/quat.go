...
func (q1 Quat ) Normalize() Quat  {
	...
	if length == float32(InfPos ){
		length =float32( MaxValue)
	}

	return Quat {float32(q1.W * 1 / length), q1.V.Mul(float32(1 / length))}
}
...
func (q1 Quat ) OrientationEqual(q2 Quat ) bool {
	return q1.OrientationEqualThreshold(q2, float32(Epsilon))
}
