...
func robustNormalWithLength(x, y r3.Vec) (r3.Vec, float64) {
	var pt r3.Vec
...
func projection(x, aNorm r3.Vec, aNormLen float64, a0, a1 Point) (proj, bound float64) {
...
func compareEdges(a0, a1, b0, b1 Point) bool {
	...
	return a0.Cmp(b0.Vec) == -1 || (a0 == b0 && b0.Cmp(b1.Vec) == -1)
}
...
func intersectionStable(a0, a1, b0, b1 Point) (Point, bool) {
	...
	aLen2 := a1.Sub(a0.Vec).Norm2()
	bLen2 := b1.Sub(b0.Vec).Norm2()
...
func intersectionStableSorted(a0, a1, b0, b1 Point) (Point, bool) {
	...
	aNorm := a0.Sub(a1.Vec).Cross(a0.Add(a1.Vec))
	aNormLen := aNorm.Norm()
	bLen := b1.Sub(b0.Vec).Norm()
	...
	b0Dist, b0Error := projection(b0.Vec, aNorm, aNormLen, a0, a1)
	b1Dist, b1Error := projection(b1.Vec, aNorm, aNormLen, a0, a1)
...
func intersectionExact(a0, a1, b0, b1 Point) Point {
	...
	// numerical stability.
	a0P := r3.PreciseVectorFromVector(a0.Vec)
	a1P := r3.PreciseVectorFromVector(a1.Vec)
	b0P := r3.PreciseVectorFromVector(b0.Vec)
	b1P := r3.PreciseVectorFromVector(b1.Vec)
	...
	if x == (r3.Vec{}) {
		...
		x = r3.Vec{10, 10, 10} // Greater than any valid S2Point
