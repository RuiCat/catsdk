...
func PaddedCellFromParentIJ(parent *PaddedCell, i, j int) *PaddedCell {
	...
	if i == 1 {
		p.bound.X.Min = middle.X.Min
	} else {
		p.bound.X.Max = middle.X.Max
	}
	if j == 1 {
		p.bound.Y.Min = middle.Y.Min
	} else {
		p.bound.Y.Max = middle.Y.Max
	}
...
func (p *PaddedCell) ShrinkToFit(rect r2.Rect) CellID {
	...
	if iMin < stToIJ(uvToST(padded.X.Min)) {
		iMin = stToIJ(uvToST(padded.X.Min))
	}
	if a, b := p.iLo+ijSize-1, stToIJ(uvToST(padded.X.Max)); a <= b {
		iXor = iMin ^ a
	} else {
		iXor = iMin ^ b
	}

	if jMin < stToIJ(uvToST(padded.Y.Min)) {
		jMin = stToIJ(uvToST(padded.Y.Min))
	}
	if a, b := p.jLo+ijSize-1, stToIJ(uvToST(padded.Y.Max)); a <= b {
		jXor = jMin ^ a
	} else {
		jXor = jMin ^ b
	}
