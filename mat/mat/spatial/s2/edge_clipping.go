...
func ClipToPaddedFace(a, b Point, f int, padding float64) (aUV, bUV r2.Point, intersects bool) {
	...
	if face(a.Vec) == f && face(b.Vec) == f {
		au, av := validFaceXYZToUV(f, a.Vec)
		bu, bv := validFaceXYZToUV(f, b.Vec)
	...
	scaleUV := 1 + padding
	scaledN := pointUVW{r3.Vec{X: scaleUV * normUVW.X, Y: scaleUV * normUVW.Y, Z: normUVW.Z}}
	...
	aTan := pointUVW{normUVW.Cross(aUVW.Vec)}
	bTan := pointUVW{bUVW.Cross(normUVW.Vec)}
...
func clipDestination(a, b, scaledN, aTan, bTan pointUVW, scaleUV float64) (r2.Point, int) {
	...
	p := pointUVW(Point{r3.Vec{uv.X, uv.Y, 1.0}})
	...
	score := 0
	if p.Sub(a.Vec).Dot(aTan.Vec) < 0 {
		score = 2 // B' is on wrong side of A.
	} else if p.Sub(b.Vec).Dot(bTan.Vec) < 0 {
...
func clipBoundAxis(a0, b0 float64, bound0 r1.Interval, a1, b1 float64, bound1 r1.Interval,
	...
	if bound0.Min < clip.Min {
		// If the upper bound is below the clips lower bound, there is nothing to do.
		if bound0.Max < clip.Min {
			return bound0, bound1, false
		}
		// narrow the intervals lower bound to the clip bound.
		bound0.Min = clip.Min
		if bound1, updated = updateEndpoint(bound1, negSlope, interpolateFloat64(clip.Min, a0, b0, a1, b1)); !updated {
			return bound0, bound1, false
		}
	}

	if bound0.Max > clip.Max {
		// If the lower bound is above the clips upper bound, there is nothing to do.
		if bound0.Min > clip.Max {
			return bound0, bound1, false
		}
		// narrow the intervals upper bound to the clip bound.
		bound0.Max = clip.Max
		if bound1, updated = updateEndpoint(bound1, !negSlope, interpolateFloat64(clip.Max, a0, b0, a1, b1)); !updated {
			return bound0, bound1, false
		}
	}
...
func FaceSegments(a, b Point) []FaceSegment {
	...
	var aFace, bFace int
	aFace, segment.a.X, segment.a.Y = xyzToFaceUV(a.Vec)
	bFace, segment.b.X, segment.b.Y = xyzToFaceUV(b.Vec)
	if aFace == bFace {
		segment.face = aFace
...
func moveOriginToValidFace(face int, a, ab Point, aUV r2.Point) (int, r2.Point) {
	...
	if n.intersectsFace() {
		...
		aTangent := ab.Normalize().Cross(a.Vec)

		// We can use the given face.
		if exit.Sub(a.Vec).Dot(aTangent) >= -faceClipErrorRadians {
			return face, aUV
		}
	}
	...
	aUV.X, aUV.Y = validFaceXYZToUV(face, a.Vec)
