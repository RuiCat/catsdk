...
func (c *CrossingEdgeQuery) computeCellsIntersected(pcell *PaddedCell, edgeBound r2.Rect) {
	...
	if edgeBound.X.Max < center.X {
		// Edge is entirely contained in the two left children.
		c.clipVAxis(edgeBound, center.Y, 0, pcell)
		return
	} else if edgeBound.X.Min >= center.X {
	...
	} else if edgeBound.Y.Min >= center.Y {
...
func splitBound(edgeBound r2.Rect, uEnd, vEnd int, u, v float64) [2]r2.Rect {
	...
	if uEnd == 1 {
		childBounds[0].X.Min = u
		childBounds[1].X.Max = u
	} else {
		childBounds[0].X.Max = u
		childBounds[1].X.Min = u
	}

	if vEnd == 1 {
		childBounds[0].Y.Min = v
		childBounds[1].Y.Max = v
	} else {
		childBounds[0].Y.Max = v
		childBounds[1].Y.Min = v
	}
