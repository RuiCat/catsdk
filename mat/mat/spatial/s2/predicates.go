...
func Sign(a, b, c Point) bool {
	...
	//     (1) x ⨯ y == -(y ⨯ x)
	//     (2) -x · y == -(x · y)
	return c.Cross(a.Vec).Dot(b.Vec) > 0
}
...
func stableSign(a, b, c Point) Direction {
	...
	if ab2 >= bc2 && ab2 >= ca2 {
		...
		e1, e2, op = ca, bc, c.Vec
	} else if bc2 >= ca2 {
		// BC is the longest edge.
		e1, e2, op = ab, ca, a.Vec
	} else {
		// CA is the longest edge.
		e1, e2, op = bc, ab, b.Vec
	}
...
func exactSign(a, b, c Point, perturb bool) Direction {
	...
	xa := r3.PreciseVectorFromVector(pa.Vec)
	xb := r3.PreciseVectorFromVector(pb.Vec)
	xc := r3.PreciseVectorFromVector(pc.Vec)
...
func CompareDistances(x, a, b Point) int {
	...
	sign = exactCompareDistances(r3.PreciseVectorFromVector(x.Vec), r3.PreciseVectorFromVector(a.Vec), r3.PreciseVectorFromVector(b.Vec))
	if sign != 0 {
		return sign
	}
...
func sin2Distance(x, y Point) (sin2, err float64) {
	...
	n := x.Sub(y.Vec).Cross(x.Add(y.Vec))
	sin2 = 0.25 * n.Norm2()
...
func CompareDistance(x, y Point, r s1.ChordAngle) int {
	...
	return exactCompareDistance(r3.PreciseVectorFromVector(x.Vec), r3.PreciseVectorFromVector(y.Vec), big.NewFloat(float64(r)).SetPrec(big.MaxPrec))
}
