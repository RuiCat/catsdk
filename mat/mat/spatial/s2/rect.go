...
func (r Rect) IsValid() bool {
	return math.Abs(r.Lat.Min) <= math.Pi/2 &&
		math.Abs(r.Lat.Max) <= math.Pi/2 &&
		r.Lng.IsValid() &&
...
func (r Rect) IsPoint() bool { return r.Lat.Min == r.Lat.Max && r.Lng.Min == r.Lng.Max }
...
func (r Rect) Lo() LatLng {
	return LatLng{s1.Angle(r.Lat.Min) * s1.Radian, s1.Angle(r.Lng.Min) * s1.Radian}
}

// Hi returns the other corner of the rectangle.
func (r Rect) Hi() LatLng {
	return LatLng{s1.Angle(r.Lat.Max) * s1.Radian, s1.Angle(r.Lng.Max) * s1.Radian}
}
...
func (r Rect) Area() float64 {
	...
	capDiff := math.Abs(math.Sin(r.Lat.Max) - math.Sin(r.Lat.Min))
...
func (r Rect) PolarClosure() Rect {
	if r.Lat.Min == -math.Pi/2 || r.Lat.Max == math.Pi/2 {
...
func (r Rect) CapBound() Cap {
	...
	if r.Lat.Max+r.Lat.Min < 0 {
		// South pole axis yields smaller cap.
		poleZ = -1
		poleAngle = math.Pi/2 + r.Lat.Max
	} else {
		poleZ = 1
		poleAngle = math.Pi/2 - r.Lat.Min
	}
	poleCap := CapFromCenterAngle(Point{r3.Vec{0, 0, poleZ}}, s1.Angle(poleAngle)*s1.Radian)

	// For bounding rectangles that span 180 degrees or less in longitude, the
	// maximum cap size is achieved at one of the rectangle vertices.  For
	// rectangles that are larger than 180 degrees, we punt and always return a
	// bounding cap centered at one of the two poles.
	if math.Remainder(r.Lng.Max-r.Lng.Min, 2*math.Pi) >= 0 && r.Lng.Max-r.Lng.Min < 2*math.Pi {
...
func intersectsLatEdge(a, b Point, lat s1.Angle, lng s1.Interval) bool {
	...
	abTheta := s1.IntervalFromPointPair(
		math.Atan2(a.Dot(y.Vec), a.Dot(x)),
		math.Atan2(b.Dot(y.Vec), b.Dot(x)))
...
func intersectsLngEdge(a, b Point, lat r1.Interval, lng s1.Angle) bool {
	...
	return CrossingSign(a, b, PointFromLatLng(LatLng{s1.Angle(lat.Min), lng}),
		PointFromLatLng(LatLng{s1.Angle(lat.Max), lng})) == Cross
}
...
func (r Rect) IntersectsCell(c Cell) bool {
	...
	for i := range vertices {
		...
		a := vertices[i]
		b := vertices[(i+1)&3]
		if edgeLng.Contains(r.Lng.Min) && intersectsLngEdge(a, b, r.Lat, s1.Angle(r.Lng.Min)) {
			return true
		}
		if edgeLng.Contains(r.Lng.Max) && intersectsLngEdge(a, b, r.Lat, s1.Angle(r.Lng.Max)) {
			return true
		}
		if intersectsLatEdge(a, b, s1.Angle(r.Lat.Min), r.Lng) {
			return true
		}
		if intersectsLatEdge(a, b, s1.Angle(r.Lat.Max), r.Lng) {
			return true
		}
	}
	return false
...
func (r Rect) encode(e *encoder) {
	...
	e.writeFloat64(r.Lat.Min)
	e.writeFloat64(r.Lat.Max)
	e.writeFloat64(r.Lng.Min)
	e.writeFloat64(r.Lng.Max)
}
...
func (r *Rect) decode(d *decoder) {
	...
	r.Lat.Min = d.readFloat64()
	r.Lat.Max = d.readFloat64()
	r.Lng.Min = d.readFloat64()
	r.Lng.Max = d.readFloat64()
	return
}
...
func (r Rect) DistanceToLatLng(ll LatLng) s1.Angle {
	if r.Lng.Contains(float64(ll.Lng)) {
		return maxAngle(0, ll.Lat-s1.Angle(r.Lat.Max), s1.Angle(r.Lat.Min)-ll.Lat)
	}

	i := s1.IntervalFromEndpoints(r.Lng.Max, r.Lng.ComplementCenter())
	rectLng := r.Lng.Min
	if i.Contains(float64(ll.Lng)) {
		rectLng = r.Lng.Max
	}

	lo := LatLng{s1.Angle(r.Lat.Min) * s1.Radian, s1.Angle(rectLng) * s1.Radian}
	hi := LatLng{s1.Angle(r.Lat.Max) * s1.Radian, s1.Angle(rectLng) * s1.Radian}
...
func directedHausdorffDistance(lngDiff s1.Angle, a, b r1.Interval) s1.Angle {
	...
	bLng := lngDiff
	// Two endpoints of b.
	bLo := PointFromLatLng(LatLng{s1.Angle(b.Min), bLng})
	bHi := PointFromLatLng(LatLng{s1.Angle(b.Max), bLng})

	// Cases A1 and B1.
	aLo := PointFromLatLng(LatLng{s1.Angle(a.Min), 0})
	aHi := PointFromLatLng(LatLng{s1.Angle(a.Max), 0})
	...
	if a.Contains(float64(pLat)) {
		maxDistance = maxAngle(maxDistance, p.Angle(bLo.Vec))
	}

	// Case B3.
	if pLat > s1.Angle(a.Min) {
		intDist, ok := interiorMaxDistance(r1.Interval{a.Min, math.Min(float64(pLat), a.Max)}, bLo)
		if ok {
			maxDistance = maxAngle(maxDistance, intDist)
		}
	}
	if pLat < s1.Angle(a.Max) {
		intDist, ok := interiorMaxDistance(r1.Interval{math.Max(float64(pLat), a.Min), a.Max}, bHi)
		if ok {
			maxDistance = maxAngle(maxDistance, intDist)
		}
	}
...
func interiorMaxDistance(aLat r1.Interval, b Point) (a s1.Angle, ok bool) {
	...
	return b.Angle(intersectionPoint.Vec), true
}
...
func (r Rect) Centroid() Point {
	...
	z1 := math.Sin(r.Lat.Min)
	z2 := math.Sin(r.Lat.Max)
	r1 := math.Cos(r.Lat.Min)
	r2 := math.Cos(r.Lat.Max)
	...
	return Point{r3.Vec{r0 * math.Cos(lng), r0 * math.Sin(lng), z}}
}
