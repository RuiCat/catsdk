...
var (
	emptyLoopPoint = Point{r3.Vec{X: 0, Y: 0, Z: 1}}
	fullLoopPoint  = Point{r3.Vec{X: 0, Y: 0, Z: -1}}
)
...
func (l *Loop) initBound() {
	...
	if l.ContainsPoint(Point{r3.Vec{0, 0, 1}}) {
		b = Rect{r1.Interval{b.Lat.Min, math.Pi / 2}, s1.FullInterval()}
	}
	...
	// b.Lng.IsFull().
	if b.Lng.IsFull() && l.ContainsPoint(Point{r3.Vec{0, 0, -1}}) {
		b.Lat.Min = -math.Pi / 2
	}
	l.bound = b
...
func (l *Loop) CanonicalFirstVertex() (firstIdx, direction int) {
	...
	n := len(l.vertices)
	for i := 1; i < n; i++ {
		if l.Vertex(i).Cmp(l.Vertex(firstIdx).Vec) == -1 {
			firstIdx = i
		}
	}

	// 0 <= firstIdx <= n-1, so (firstIdx+n*dir) <= 2*n-1.
	if l.Vertex(firstIdx+1).Cmp(l.Vertex(firstIdx+n-1).Vec) == -1 {
		return firstIdx, 1
	}
...
func (l *Loop) Invert() {
	...
	if l.bound.Lat.Min > -math.Pi/2 && l.bound.Lat.Max < math.Pi/2 {
...
func (l *Loop) surfaceIntegralFloat64(f func(a, b, c Point) float64) float64 {
	...
	for i := 1; i+1 < len(l.vertices); i++ {
		...
		//     (O, V_0, V_1, ..., V_i).
		if l.Vertex(i+1).Angle(origin.Vec) > maxLength {
			...
			if origin == l.Vertex(0) {
			...
			} else if l.Vertex(i).Angle(l.Vertex(0).Vec) < maxLength {
				...
				origin = Point{l.Vertex(0).Cross(oldOrigin.Vec)}
	...
	for i := 1; i+1 < len(l.vertices); i++ {
		if l.Vertex(i+1).Angle(origin.Vec) > maxLength {
			...
			if origin == l.Vertex(0) {
				...
				origin = Point{l.Vertex(0).Cross(oldOrigin.Vec)}
				sum = sum.Add(f(l.Vertex(0), oldOrigin, origin).Vec)
			}
			sum = sum.Add(f(oldOrigin, l.Vertex(i), origin).Vec)
		}
		sum = sum.Add(f(origin, l.Vertex(i), l.Vertex(i+1)).Vec)
	}
	if origin != l.Vertex(0) {
		sum = sum.Add(f(origin, l.Vertex(len(l.vertices)-1), l.Vertex(0)).Vec)
	}
	return Point{sum}
}
