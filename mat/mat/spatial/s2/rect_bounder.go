...
func (r *RectBounder) AddPoint(b Point) {
	...
	n := r.a.Sub(b.Vec).Cross(r.a.Add(b.Vec)) // N = 2 * (A x B)
	...
	if mA*mB < 0 || math.Abs(mA) <= mError || math.Abs(mB) <= mError {
		...
		latBudget := 2 * math.Asin(0.5*(r.a.Sub(b.Vec)).Norm()*math.Sin(maxLat))
		...
		if mA <= mError && mB >= -mError {
			latAB.Max = math.Min(maxLat, latAB.Max+maxDelta)
		}
		if mB <= mError && mA >= -mError {
			latAB.Min = math.Max(-maxLat, latAB.Min-maxDelta)
		}
	}
	r.a = b
	r.aLL = bLL
...
func ExpandForSubregions(bound Rect) Rect {
	...
	minAbsLat := math.Max(bound.Lat.Min, -bound.Lat.Max)

	// latGapSouth and latGapNorth measure the minimum distance from B to the
	// south and north poles respectively.
	latGapSouth := math.Pi/2 + bound.Lat.Min
	latGapNorth := math.Pi/2 - bound.Lat.Max
