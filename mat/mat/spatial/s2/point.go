...
func (p points) Less(i, j int) bool { return p[i].Cmp(p[j].Vec) == -1 }
...
func PointFromCoords(x, y, z float64) Point {
	...
	return Point{r3.Vec{x, y, z}.Normalize()}
}
...
func OriginPoint() Point {
	return Point{r3.Vec{-0.0099994664350250197, 0.0025924542609324121, 0.99994664350250195}}
}
...
func (p Point) PointCross(op Point) Point {
	...
	x := p.Add(op.Vec).Cross(op.Sub(p.Vec))
...
func (p Point) approxEqual(other Point, eps s1.Angle) bool {
	return p.Vec.Angle(other.Vec) <= eps
}
...
func ChordAngleBetweenPoints(x, y Point) s1.ChordAngle {
	return s1.ChordAngle(math.Min(4.0, x.Sub(y.Vec).Norm2()))
}
...
func regularPointsForFrame(frame matrix3x3, radius s1.Angle, numVertices int) []Point {
	...
	for i := 0; i < numVertices; i++ {
		...
		p := Point{r3.Vec{r * math.Cos(angle), r * math.Sin(angle), z}}
...
func Ortho(a Point) Point {
	temp := r3.Vec{0.012, 0.0053, 0.00457}
...
func (p Point) stableAngle(o Point) s1.Angle {
	return s1.Angle(2 * math.Atan2(p.Sub(o.Vec).Norm(), p.Add(o.Vec).Norm()))
}
