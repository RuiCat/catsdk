...
func (c Cell) Edge(k int) Point {
	switch k {
	case 0:
		return Point{vNorm(int(c.face), c.uv.Y.Min).Normalize()} // Bottom
	case 1:
...
func (c Cell) Children() ([4]Cell, bool) {
	...
	for pos := 0; pos < 4; pos++ {
		...
		j := ij & 1
		if i == 1 {
			children[pos].uv.X.Max = c.uv.X.Max
			children[pos].uv.X.Min = uvMid.X
		} else {
			children[pos].uv.X.Min = c.uv.X.Min
			children[pos].uv.X.Max = uvMid.X
		}
		if j == 1 {
			children[pos].uv.Y.Max = c.uv.Y.Max
			children[pos].uv.Y.Min = uvMid.Y
		} else {
			children[pos].uv.Y.Min = c.uv.Y.Min
			children[pos].uv.Y.Max = uvMid.Y
		}
		cid = cid.Next()
	}
...
func (c Cell) ApproxArea() float64 {
	...
	flatArea := 0.5 * (c.Vertex(2).Sub(c.Vertex(0).Vec).
		Cross(c.Vertex(3).Sub(c.Vertex(1).Vec)).Norm())
...
func (c Cell) RectBound() Rect {
	if c.level > 0 {
		...
		u := c.uv.X.Min + c.uv.X.Max
		v := c.uv.Y.Min + c.uv.Y.Max
		var i, j int
...
func (c Cell) uEdgeIsClosest(p Point, vHi bool) bool {
	...
	dir0 := r3.Vec{v*v + 1, -u0 * v, -u0}
	dir1 := r3.Vec{v*v + 1, -u1 * v, -u1}
...
func (c Cell) vEdgeIsClosest(p Point, uHi bool) bool {
	...
	dir0 := r3.Vec{-u * v0, u*u + 1, -v0}
	dir1 := r3.Vec{-u * v1, u*u + 1, -v1}
	return p.Dot(dir0) > 0 && p.Dot(dir1) < 0
}
...
func (c Cell) distanceInternal(targetXYZ Point, toInterior bool) s1.ChordAngle {
	...
	if inside {
		...
		return minChordAngle(edgeDistance(-dir00, c.uv.X.Min),
			edgeDistance(dir01, c.uv.X.Max),
			edgeDistance(-dir10, c.uv.Y.Min),
			edgeDistance(dir11, c.uv.Y.Max))
	}
