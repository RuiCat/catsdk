...
func GirardArea(a, b, c Point) float64 {
	...
	area := float64(ab.Angle(ac.Vec) - ab.Angle(bc.Vec) + bc.Angle(ac.Vec))
	if area < 0 {
		area = 0
	}
	return area
...
func Angle(a, b, c Point) s1.Angle {
	...
	return a.PointCross(b).Angle(c.PointCross(b).Vec)
}
...
func TurnAngle(a, b, c Point) s1.Angle {
	...
	angle := a.PointCross(b).Angle(b.PointCross(c).Vec)
