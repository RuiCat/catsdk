...
func (s *ShapeIndex) addFaceEdge(fe faceEdge, allEdges [][]faceEdge) {
	...
	if aFace == face(fe.edge.V1.Vec) {
		x, y := validFaceXYZToUV(aFace, fe.edge.V0.Vec)
		fe.a = r2.Point{x, y}
		x, y = validFaceXYZToUV(aFace, fe.edge.V1.Vec)
		fe.b = r2.Point{x, y}
...
func (s *ShapeIndex) updateEdges(pcell *PaddedCell, edges []*clippedEdge, t *tracker, disjointFromIndex bool) {
	...
	if !disjointFromIndex || !s.makeIndexCell(pcell, edges, t) {
		...
		for _, edge := range edges {
			if edge.bound.X.Max <= middle.X.Min {
			...
			} else if edge.bound.X.Min >= middle.X.Max {
			...
			} else if edge.bound.Y.Max <= middle.Y.Min {
				// Edge is entirely contained in the two lower children.
				if a := s.clipUBound(edge, 1, middle.X.Max); a != nil {
					childEdges[0][0] = append(childEdges[0][0], a)
				}
				if b := s.clipUBound(edge, 0, middle.X.Min); b != nil {
					childEdges[1][0] = append(childEdges[1][0], b)
				}
			} else if edge.bound.Y.Min >= middle.Y.Max {
				...
				left := s.clipUBound(edge, 1, middle.X.Max)
				...
				right := s.clipUBound(edge, 0, middle.X.Min)
...
func (s *ShapeIndex) updateBound(edge *clippedEdge, uEnd int, u float64, vEnd int, v float64) *clippedEdge {
	...
	if uEnd == 0 {
		c.bound.X.Min = u
		c.bound.X.Max = edge.bound.X.Max
	} else {
		c.bound.X.Min = edge.bound.X.Min
		c.bound.X.Max = u
	}

	if vEnd == 0 {
		c.bound.Y.Min = v
		c.bound.Y.Max = edge.bound.Y.Max
	} else {
		c.bound.Y.Min = edge.bound.Y.Min
		c.bound.Y.Max = v
	}
...
func (s *ShapeIndex) clipUBound(edge *clippedEdge, uEnd int, u float64) *clippedEdge {
	...
	if uEnd == 0 {
		if edge.bound.X.Min >= u {
			return edge
		}
	} else {
		if edge.bound.X.Max <= u {
			return edge
		}
	}
...
func (s *ShapeIndex) clipVBound(edge *clippedEdge, vEnd int, v float64) *clippedEdge {
	if vEnd == 0 {
		if edge.bound.Y.Min >= v {
			return edge
		}
	} else {
		if edge.bound.Y.Max <= v {
			return edge
		}
	}
...
func (s *ShapeIndex) clipVAxis(edge *clippedEdge, middle r1.Interval) (a, b *clippedEdge) {
	if edge.bound.Y.Max <= middle.Min {
		// Edge is entirely contained in the lower child.
		return edge, nil
	} else if edge.bound.Y.Min >= middle.Max {
		// Edge is entirely contained in the upper child.
		return nil, edge
	}
	// The edge bound spans both children.
	return s.clipVBound(edge, 1, middle.Max), s.clipVBound(edge, 0, middle.Min)
}
...
func maxLevelForEdge(edge Edge) int {
	...
	cellSize := edge.V0.Sub(edge.V1.Vec).Norm() * cellSizeToLongEdgeRatio
