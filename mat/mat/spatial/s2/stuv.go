...
func validFaceXYZToUV(face int, r r3.Vec) (float64, float64) {
	switch face {
	case 0:
...
func xyzToFaceUV(r r3.Vec) (f int, u, v float64) {
	f = face(r)
...
func faceXYZToUV(face int, p Point) (u, v float64, ok bool) {
	...
	u, v = validFaceXYZToUV(face, p.Vec)
	return u, v, true
}
...
func faceXYZtoUVW(face int, p Point) Point {
	...
	switch face {
	case 0:
		return Point{r3.Vec{p.Y, p.Z, p.X}}
	case 1:
		return Point{r3.Vec{-p.X, p.Z, p.Y}}
	case 2:
		return Point{r3.Vec{-p.X, -p.Y, p.Z}}
	case 3:
		return Point{r3.Vec{-p.Z, -p.Y, -p.X}}
	case 4:
		return Point{r3.Vec{-p.Z, p.X, -p.Y}}
	default:
		return Point{r3.Vec{p.Y, p.X, -p.Z}}
	}
}
...
func xyzToFaceSiTi(p Point) (face int, si, ti uint32, level int) {
	...
	if p.Vec == faceSiTiToXYZ(face, si, ti).Normalize() {
...
func uNorm(face int, u float64) r3.Vec {
	switch face {
	case 0:
		return r3.Vec{u, -1, 0}
	case 1:
...
// the given face.
func vNorm(face int, v float64) r3.Vec {
	switch face {
	case 0:
		return r3.Vec{-v, 0, 1}
	case 1:
...
var faceUVWAxes = [6][3]Point{
	{Point{r3.Vec{0, 1, 0}}, Point{r3.Vec{0, 0, 1}}, Point{r3.Vec{1, 0, 0}}},
	{Point{r3.Vec{-1, 0, 0}}, Point{r3.Vec{0, 0, 1}}, Point{r3.Vec{0, 1, 0}}},
	{Point{r3.Vec{-1, 0, 0}}, Point{r3.Vec{0, -1, 0}}, Point{r3.Vec{0, 0, 1}}},
	{Point{r3.Vec{0, 0, -1}}, Point{r3.Vec{0, -1, 0}}, Point{r3.Vec{-1, 0, 0}}},
	{Point{r3.Vec{0, 0, -1}}, Point{r3.Vec{1, 0, 0}}, Point{r3.Vec{0, -1, 0}}},
	{Point{r3.Vec{0, 1, 0}}, Point{r3.Vec{1, 0, 0}}, Point{r3.Vec{0, 0, -1}}},
}
