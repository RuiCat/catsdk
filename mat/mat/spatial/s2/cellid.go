...
func cellIDFromPoint(p Point) CellID {
	f, u, v := xyzToFaceUV(r3.Vec{p.X, p.Y, p.Z})
	i := stToIJ(uvToST(u))
	j := stToIJ(uvToST(v))
	return cellIDFromFaceIJ(f, i, j)
}
...
func ijLevelToBoundUV(i, j, level int) r2.Rect {
	...
	return r2.Rect{
		X: r1.Interval{
			Min: stToUV(ijToSTMin(xLo)),
			Max: stToUV(ijToSTMin(xLo + cellSize)),
		},
		Y: r1.Interval{
			Min: stToUV(ijToSTMin(yLo)),
			Max: stToUV(ijToSTMin(yLo + cellSize)),
		},
	}
}
...
func expandedByDistanceUV(uv r2.Rect, distance s1.Angle) r2.Rect {
	...
	maxU := math.Max(math.Abs(uv.X.Min), math.Abs(uv.X.Max))
	maxV := math.Max(math.Abs(uv.Y.Min), math.Abs(uv.Y.Max))
	sinDist := math.Sin(float64(distance))
	return r2.Rect{
		X: r1.Interval{expandEndpoint(uv.X.Min, maxV, -sinDist),
			expandEndpoint(uv.X.Max, maxV, sinDist)},
		Y: r1.Interval{expandEndpoint(uv.Y.Min, maxU, -sinDist),
			expandEndpoint(uv.Y.Max, maxU, sinDist)}}
}
