...
func (c Cap) IsValid() bool {
	return c.center.Vec.IsUnit() && c.radius <= s1.StraightChordAngle
}
...
func (c Cap) RectBound() Rect {
	...
	allLongitudes := false
	lat := r1.Interval{
		Min: latitude(c.center).Radians() - capAngle,
		Max: latitude(c.center).Radians() + capAngle,
	}
	lng := s1.FullInterval()

	// Check whether cap includes the south pole.
	if lat.Min <= -math.Pi/2 {
		lat.Min = -math.Pi / 2
		allLongitudes = true
	}

	// Check whether cap includes the north pole.
	if lat.Max >= math.Pi/2 {
		lat.Max = math.Pi / 2
		allLongitudes = true
	}
	...
	if !allLongitudes {
		...
		if sinA <= sinC {
			...
			lng.Min = math.Remainder(longitude(c.center).Radians()-angleA, math.Pi*2)
			lng.Max = math.Remainder(longitude(c.center).Radians()+angleA, math.Pi*2)
		}
	}
	return Rect{lat, lng}
}
...
func (c Cap) String() string {
	return fmt.Sprintf("[Center=%v, Radius=%f]", c.center.Vec, c.Radius().Degrees())
}
...
func (c Cap) intersects(cell Cell, vertices [4]Point) bool {
	...
	for k := 0; k < 4; k++ {
		edge := cell.Edge(k).Vec
		dot := c.center.Vec.Dot(edge)
		if dot > 0 {
		...
		dir := edge.Cross(c.center.Vec)
		if dir.Dot(vertices[k].Vec) < 0 && dir.Dot(vertices[(k+1)&3].Vec) > 0 {
			return true
		}
	}
	return false
