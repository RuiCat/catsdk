...
// stack.
func (ms *TransformStack ) Push(m f32.Mat4 ) {
	prev := (*ms)[len(*ms)-1]
...
func (ms *TransformStack ) Pop() (f32.Mat4 , error) {
	if len(*ms) == 1 {
		return f32.Mat4 {}, errors.New("attempt to pop last element of the stack; Matrix Stack must have at least one element")
	}
...
// removing it.
func (ms *TransformStack ) Peek() f32.Mat4  {
	return (*ms)[len(*ms)-1]
}
...
func (ms *TransformStack ) Reseed(n int, change f32.Mat4 ) error {
	if n >= len(*ms) || n <= 0 {
...
func (ms *TransformStack ) reseed(n int, change f32.Mat4 ) error {
	backup := []f32.Mat4 ((*ms)[n:])
	backup = append([]f32.Mat4 {}, backup...) // copy into new slice
...
func (ms *TransformStack ) undoRebase(n int, prev []f32.Mat4 ) {
	for i := n; i < len(*ms); i++ {
		(*ms)[i] = prev[i-n]
	}
}
