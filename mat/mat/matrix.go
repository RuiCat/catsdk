...
func Cond(a Matrix, norm float64) float64 {
	...
	var lnorm blas.MatrixNorm
	switch norm {
	default:
		panic("mat: bad norm value")
	case 1:
		lnorm = blas.MaxColumnSum
	case 2:
		var svd SVD
...
// normLapack converts the float64 norm input in Norm to a blas.MatrixNorm.
func normLapack(norm float64, aTrans bool) blas.MatrixNorm {
	switch norm {
	case 1:
		n := blas.MaxColumnSum
		if aTrans {
