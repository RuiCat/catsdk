...
import (
	"mat/blas"
	"mat/blas/blas64"
	"mat/blas/lapack/gonum"
)

var lapack64 blas.Lapack = gonum.Implementation{}
...
func Gecon(norm blas.MatrixNorm, a blas64.General, anorm float64, work []float64, iwork []int) float64 {
...
// is as follows
//
//	jobU == blas.SVDAll       All m columns of U are returned in u
//	jobU == blas.SVDStore     The first min(m,n) columns are returned in u
//	jobU == blas.SVDOverwrite The first min(m,n) columns of U are written into a
//	jobU == blas.SVDNone      The columns of U are not computed.
//
// The behavior is the same for jobVT and the rows of Vᵀ. At most one of jobU
// and jobVT can equal blas.SVDOverwrite, and Gesvd will panic otherwise.
//
...
// if either job is blas.SVDOverwrite.
//
...
// jobU == blas.SVDAll, u is of size m×m. If jobU == blas.SVDStore u is
// of size m×min(m,n). If jobU == blas.SVDOverwrite or blas.SVDNone, u is
// not used.
//
// vt contains the left singular vectors on exit, stored rowwise. If
// jobV == blas.SVDAll, vt is of size n×m. If jobVT == blas.SVDStore vt is
// of size min(m,n)×n. If jobVT == blas.SVDOverwrite or blas.SVDNone, vt is
// not used.
//
...
func Gesvd(jobU, jobVT blas.SVDJob, a, u, vt blas64.General, s, work []float64, lwork int) (ok bool) {
...
// is as follows
//
//	jobU == blas.GSVDU        Compute orthogonal matrix U
//	jobU == blas.GSVDNone     Do not compute orthogonal matrix.
//
// The behavior is the same for jobV and jobQ with the exception that instead of
// blas.GSVDU these accept blas.GSVDV and blas.GSVDQ respectively.
// The matrices U, V and Q must be m×m, p×p and n×n respectively unless the
// relevant job parameter is blas.GSVDNone.
//
...
func Ggsvd3(jobU, jobV, jobQ blas.GSVDJob, a, b blas64.General, alpha, beta []float64, u, v, q blas64.General, work []float64, lwork int, iwork []int) (k, l int, ok bool) {
...
//
// Dgtsv is not part of the blas.Float64 interface and so calls to Gtsv are
...
//
// Dlagtm is not part of the blas.Float64 interface and so calls to Lagtm are
// always executed by the Gonum implementation.
...
//
//	blas.MaxAbs: the maximum absolute value of an element.
//	blas.MaxColumnSum: the maximum column sum of the absolute values of the entries.
//	blas.MaxRowSum: the maximum row sum of the absolute values of the entries.
//	blas.Frobenius: the square root of the sum of the squares of the entries.
//
// If norm == blas.MaxColumnSum, work must be of length n, and this function will panic otherwise.
// There are no restrictions on work for the other matrix norms.
func Lange(norm blas.MatrixNorm, a blas64.General, work []float64) float64 {
...
// ku super-diagonals.
//
// Dlangb is not part of the blas.Float64 interface and so calls to Langb are always
// executed by the Gonum implementation.
func Langb(norm blas.MatrixNorm, a blas64.Band) float64 {
...
//
// Dlangt is not part of the blas.Float64 interface and so calls to Langt are
// always executed by the Gonum implementation.
func Langt(norm blas.MatrixNorm, a Tridiagonal) float64 {
...
// There are no restrictions on work for the other matrix norms.
//
// Dlansb is not part of the blas.Float64 interface and so calls to Lansb are always
// executed by the Gonum implementation.
func Lansb(norm blas.MatrixNorm, a blas64.SymmetricBand, work []float64) float64 {
...
// at least n and this function will panic otherwise.
// There are no restrictions on work for the other matrix norms.
func Lansy(norm blas.MatrixNorm, a blas64.Symmetric, work []float64) float64 {
...
// norm == blas.MaxColumnSum work must have length at least n and this function
// will panic otherwise. There are no restrictions on work for the other matrix norms.
func Lantr(norm blas.MatrixNorm, a blas64.Triangular, work []float64) float64 {
...
// norms.
func Lantb(norm blas.MatrixNorm, a blas64.TriangularBand, work []float64) float64 {
...
// portion specified by uplo. If jobz == blas.EVCompute, a contains the
// orthonormal eigenvectors of A on exit, otherwise jobz must be blas.EVNone
...
func Syev(jobz blas.EVJob, a blas64.Symmetric, w, work []float64, lwork int) (ok bool) {
...
func Trcon(norm blas.MatrixNorm, a blas64.Triangular, work []float64, iwork []int) float64 {
...
//
// Left eigenvectors will be computed only if jobvl == blas.LeftEVCompute,
// otherwise jobvl must be blas.LeftEVNone.
// Right eigenvectors will be computed only if jobvr == blas.RightEVCompute,
// otherwise jobvr must be blas.RightEVNone.
...
func Geev(jobvl blas.LeftEVJob, jobvr blas.RightEVJob, a blas64.General, wr, wi []float64, vl, vr blas64.General, work []float64, lwork int) (first int) {
	n := a.Rows
	if a.Cols != n {
		panic("lapack64: matrix not square")
	}
	if jobvl == blas.LeftEVCompute && (vl.Rows != n || vl.Cols != n) {
		panic("lapack64: bad size of VL")
	}
	if jobvr == blas.RightEVCompute && (vr.Rows != n || vr.Cols != n) {
