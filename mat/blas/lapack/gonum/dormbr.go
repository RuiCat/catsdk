...
//
//	Q * C   if vect == blas.ApplyQ, side == blas.Left, and trans == blas.NoTrans
//	C * Q   if vect == blas.ApplyQ, side == blas.Right, and trans == blas.NoTrans
//	Qᵀ * C  if vect == blas.ApplyQ, side == blas.Left, and trans == blas.Trans
//	C * Qᵀ  if vect == blas.ApplyQ, side == blas.Right, and trans == blas.Trans
//
//	P * C   if vect == blas.ApplyP, side == blas.Left, and trans == blas.NoTrans
//	C * P   if vect == blas.ApplyP, side == blas.Right, and trans == blas.NoTrans
//	Pᵀ * C  if vect == blas.ApplyP, side == blas.Left, and trans == blas.Trans
//	C * Pᵀ  if vect == blas.ApplyP, side == blas.Right, and trans == blas.Trans
//
...
//
// If vect == blas.ApplyQ, A is assumed to have been an nq×k matrix, while if
// vect == blas.ApplyP, A is assumed to have been a k×nq matrix. nq = m if
...
func (impl Implementation) Dormbr(vect blas.ApplyOrtho, side blas.Side, trans blas.Transpose, m, n, k int, a []float64, lda int, tau, c []float64, ldc int, work []float64, lwork int) {
	nq := n
	nw := m
	if side == blas.Left {
		nq = m
	...
	applyQ := vect == blas.ApplyQ
	switch {
	case !applyQ && vect != blas.ApplyP:
		panic(badApplyOrtho)
