...
// with k+1 diagonals.
//
// When norm is blas.MaxColumnSum, the length of work must be at least n.
func (impl Implementation) Dlantb(norm blas.MatrixNorm, uplo blas.Uplo, diag blas.Diag, n, k int, a []float64, lda int, work []float64) float64 {
	switch {
	...
	case len(a) < (n-1)*lda+k+1:
		panic(shortAB)
	case len(work) < n && norm == blas.MaxColumnSum:
		panic(shortWork)
	}
