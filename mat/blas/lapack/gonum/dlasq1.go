...
func (impl Implementation) Dlasq1(n int, d, e, work []float64) (info int) {
	...
	if sigmx == 0 {
		impl.Dlasrt(blas.SortDecreasing, n, d)
		return info
	}
	...
	impl.Dlascl(blas.General, 0, 0, sigmx, scale, 2*n-1, 1, work, 1)
	...
	if info == 0 {
		...
		impl.Dlascl(blas.General, 0, 0, scale, sigmx, n, 1, d, 1)
	} else if info == 2 {
		...
		impl.Dlascl(blas.General, 0, 0, scale, sigmx, n, 1, d, 1)
		impl.Dlascl(blas.General, 0, 0, scale, sigmx, n, 1, e, 1)
	}
	return info
}
