...
//	jobU == blas.GSVDUnit     Use unit-initialized matrix
...
func (impl Implementation) Dtgsja(jobU, jobV, jobQ blas.GSVDJob, m, p, n, k, l int, a []float64, lda int, b []float64, ldb int, tola, tolb float64, alpha, beta, u []float64, ldu int, v []float64, ldv int, q []float64, ldq int, work []float64) (cycles int, ok bool) {
	const maxit = 40

	initu := jobU == blas.GSVDUnit
	wantu := initu || jobU == blas.GSVDU

	initv := jobV == blas.GSVDUnit
	wantv := initv || jobV == blas.GSVDV

	initq := jobQ == blas.GSVDUnit
	wantq := initq || jobQ == blas.GSVDQ

	switch {
	case !initu && !wantu && jobU != blas.GSVDNone:
		panic(badGSVDJob + "U")
	case !initv && !wantv && jobV != blas.GSVDNone:
		panic(badGSVDJob + "V")
	case !initq && !wantq && jobQ != blas.GSVDNone:
		panic(badGSVDJob + "Q")
	case m < 0:
		panic(mLT0)
	case p < 0:
