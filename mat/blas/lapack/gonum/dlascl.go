...
func (impl Implementation) Dlascl(kind blas.MatrixType, kl, ku int, cfrom, cto float64, m, n int, a []float64, lda int) {
	switch kind {
	default:
		panic(badMatrixType)
	case 'H', 'B', 'Q', 'Z': // See dlascl.f.
		panic("not implemented")
	case blas.General, blas.UpperTri, blas.LowerTri:
		if lda < max(1, n) {
			panic(badLdA)
		}
	}
