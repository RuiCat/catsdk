...
// rotations. They may either be formed explicitly (blas.OrthoExplicit), or
// they may be postmultiplied into input matrices Q1 and Z1
// (blas.OrthoPostmul), so that
//
...
func (impl Implementation) Dgghrd(compq, compz blas.OrthoComp, n, ilo, ihi int, a []float64, lda int, b []float64, ldb int, q []float64, ldq int, z []float64, ldz int) {
	switch {
	case compq != blas.OrthoNone && compq != blas.OrthoExplicit && compq != blas.OrthoPostmul:
		panic(badOrthoComp)
	case compz != blas.OrthoNone && compz != blas.OrthoExplicit && compz != blas.OrthoPostmul:
		panic(badOrthoComp)
	case n < 0:
		panic(nLT0)
	...
	case ldb < max(1, n):
		panic(badLdB)
	case (compq != blas.OrthoNone && ldq < n) || ldq < 1:
		panic(badLdQ)
	case (compz != blas.OrthoNone && ldz < n) || ldz < 1:
		panic(badLdZ)
	}
	...
	case len(b) < (n-1)*ldb+n:
		panic(shortB)
	case compq != blas.OrthoNone && len(q) < (n-1)*ldq+n:
		panic(shortQ)
	case compz != blas.OrthoNone && len(z) < (n-1)*ldz+n:
		panic(shortZ)
	}

	if compq == blas.OrthoExplicit {
		impl.Dlaset(blas.All, n, n, 0, 1, q, ldq)
	}
	if compz == blas.OrthoExplicit {
