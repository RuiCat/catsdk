...
func (impl Implementation) Dsterf(n int, d, e []float64) (ok bool) {
	...
	for {
		...
		if anorm > ssfmax {
			iscale = down
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmax, lend-l+1, 1, d[l:], n)
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmax, lend-l, 1, e[l:], n)
		} else if anorm < ssfmin {
			iscale = up
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmin, lend-l+1, 1, d[l:], n)
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmin, lend-l, 1, e[l:], n)
		}
		...
		switch iscale {
		case down:
			impl.Dlascl(blas.General, 0, 0, ssfmax, anorm, lendsv-lsv+1, 1, d[lsv:], n)
		case up:
			impl.Dlascl(blas.General, 0, 0, ssfmin, anorm, lendsv-lsv+1, 1, d[lsv:], n)
		}
