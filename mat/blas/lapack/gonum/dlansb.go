...
// super-diagonals.
//
// When norm is blas.MaxColumnSum or blas.MaxRowSum, the length of work must
// be at least n.
func (impl Implementation) Dlansb(norm blas.MatrixNorm, uplo blas.Uplo, n, kd int, ab []float64, ldab int, work []float64) float64 {
	switch {
	...
	case len(ab) < (n-1)*ldab+kd+1:
		panic(shortAB)
	case len(work) < n && (norm == blas.MaxColumnSum || norm == blas.MaxRowSum):
		panic(shortWork)
	}
	...
	case blas.MaxColumnSum, blas.MaxRowSum:
		work = work[:n]
		var sum float64
		if uplo == blas.Upper {
			for i := range work {
