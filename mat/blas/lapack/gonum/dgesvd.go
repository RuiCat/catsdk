...
// and jobVT can equal blas.SVDOverwrite, and Dgesvd will panic otherwise.
//
...
// jobV == blas.SVDAll, vt is of size n×n. If jobVT == blas.SVDStore vt is
...
func (impl Implementation) Dgesvd(jobU, jobVT blas.SVDJob, m, n int, a []float64, lda int, s, u []float64, ldu int, vt []float64, ldvt int, work []float64, lwork int) (ok bool) {
	if jobU == blas.SVDOverwrite || jobVT == blas.SVDOverwrite {
		panic(noSVDO)
	}

	wantua := jobU == blas.SVDAll
	wantus := jobU == blas.SVDStore
	wantuas := wantua || wantus
	wantuo := jobU == blas.SVDOverwrite
	wantun := jobU == blas.SVDNone
	if !(wantua || wantus || wantuo || wantun) {
		panic(badSVDJob)
	}

	wantva := jobVT == blas.SVDAll
	wantvs := jobVT == blas.SVDStore
	wantvas := wantva || wantvs
	wantvo := jobVT == blas.SVDOverwrite
	wantvn := jobVT == blas.SVDNone
	...
	if m >= n {
		...
		impl.Dorgbr(blas.GeneratePT, n, n, n, a, lda, nil, work, -1)
		lwork_dorgbr_p := int(work[0])

		impl.Dorgbr(blas.GenerateQ, n, n, n, a, lda, nil, work, -1)
		...
		if m >= mnthr {
			...
			if wantus || wantuo {
				impl.Dorgbr(blas.GenerateQ, m, n, n, a, lda, nil, work, -1)
				lwork_dorgbr_q = int(work[0])
				maxwrk = max(maxwrk, 3*n+lwork_dorgbr_q)
			}
			if wantua {
				impl.Dorgbr(blas.GenerateQ, m, m, n, a, lda, nil, work, -1)
				lwork_dorgbr_q := int(work[0])
				maxwrk = max(maxwrk, 3*n+lwork_dorgbr_q)
			}
			if !wantvn {
		...
		impl.Dorgbr(blas.GeneratePT, m, m, m, a, n, nil, work, -1)
		lwork_dorgbr_p := int(work[0])

		impl.Dorgbr(blas.GenerateQ, m, m, m, a, n, nil, work, -1)
		lwork_dorgbr_q := int(work[0])
		...
		if n >= mnthr {
			...
			if wantvs || wantvo {
				impl.Dorgbr(blas.GeneratePT, m, n, m, a, n, nil, work, -1)
				lwork_dorgbr_p = int(work[0])
				maxwrk = max(maxwrk, 3*m+lwork_dorgbr_p)
			}
			if wantva {
				impl.Dorgbr(blas.GeneratePT, n, n, m, a, n, nil, work, -1)
				lwork_dorgbr_p = int(work[0])
				maxwrk = max(maxwrk, 3*m+lwork_dorgbr_p)
			}
			if !wantun {
	...
	anrm := impl.Dlange(blas.MaxAbs, m, n, a, lda, nil)
	var iscl bool
	if anrm > 0 && anrm < smlnum {
		iscl = true
		impl.Dlascl(blas.General, 0, 0, anrm, smlnum, m, n, a, lda)
	} else if anrm > bignum {
		iscl = true
		impl.Dlascl(blas.General, 0, 0, anrm, bignum, m, n, a, lda)
	}
	...
	if m >= n {
		...
		if m >= mnthr {
			...
			if wantun {
				...
				ncvt := 0
				if wantvo || wantvas {
					impl.Dorgbr(blas.GeneratePT, n, n, n, a, lda, work[itaup:],
				...
				if wantvn {
					...
					if lwork >= n*n+max(4*n, bdspac) {
						...
						impl.Dorgbr(blas.GenerateQ, n, n, n, work[ir:], ldworkr,
						...
						impl.Dormbr(blas.ApplyQ, blas.Right, blas.NoTrans, m, n, n,
					...
					if lwork >= n*n+max(4*n, bdspac) {
						...
						impl.Dorgbr(blas.GenerateQ, n, n, n, work[iu:], ldworku,
							work[itauq:], work[iwork:], lwork-iwork)

						// Generate right bidiagonalizing vectors in VT.
						impl.Dorgbr(blas.GeneratePT, n, n, n, vt, ldvt,
							work[itaup:], work[iwork:], lwork-iwork)
						iwork = ie + n
				...
				if wantvn {
					...
					if lwork >= n*n+max(max(n+m, 4*n), bdspac) {
						...
						/*
							...
							// Multiply Q in U by left bidiagonalizing vectors in VT.
							impl.Dormbr(blas.ApplyQ, blas.Right, blas.NoTrans,
			...
			if wantuas {
				...
				impl.Dorgbr(blas.GenerateQ, m, ncu, n, u, ldu, work[itauq:], work[iwork:], lwork-iwork)
			}
			if wantvas {
				// Right singular vectors are desired in VT. Copy result to VT and
				// generate left biadiagonalizing vectors in VT.
				impl.Dlacpy(blas.Upper, n, n, a, lda, vt, ldvt)
				impl.Dorgbr(blas.GeneratePT, n, n, n, vt, ldvt, work[itaup:], work[iwork:], lwork-iwork)
			}
			if wantuo {
				panic(noSVDO)
			}
		...
		if n >= mnthr {
			...
			if wantvn {
				...
				if wantuo || wantuas {
					impl.Dorgbr(blas.GenerateQ, m, m, m, a, lda,
						work[itauq:], work[iwork:], lwork-iwork)
				}
				iwork = ie + m
				nru := 0
				...
				if wantun {
					...
					if lwork >= m*m+max(4*m, bdspac) {
						...
						impl.Dorgbr(blas.GeneratePT, m, m, m, work[ir:], ldworkr,
							work[itaup:], work[iwork:], lwork-iwork)
						iwork = ie + m
						...
						impl.Dormbr(blas.ApplyP, blas.Left, blas.Trans, m, n, m,
					...
					if lwork >= m*m+max(4*m, bdspac) {
						...
						impl.Dorgbr(blas.GeneratePT, m, m, m, work[iu:], ldworku,
							work[itaup:], work[iwork:], lwork-iwork)

						// Generate left bidiagonalizing vectors in U.
						impl.Dorgbr(blas.GenerateQ, m, m, m, u, ldu, work[itauq:], work[iwork:], lwork-iwork)
						iwork = ie + m
			...
			if wantuas {
				...
				// generate left bidiagonalizing vectors in U.
				impl.Dlacpy(blas.Lower, m, m, a, lda, u, ldu)
				impl.Dorgbr(blas.GenerateQ, m, m, n, u, ldu, work[itauq:], work[iwork:], lwork-iwork)
			}
			if wantvas {
				...
				impl.Dorgbr(blas.GeneratePT, nrvt, n, m, vt, ldvt, work[itaup:], work[iwork:], lwork-iwork)
			}
			if wantuo {
				panic(noSVDO)
			}
	...
	if iscl {
		if anrm > bignum {
			impl.Dlascl(blas.General, 0, 0, bignum, anrm, 1, minmn, s, minmn)
		}
		if !ok && anrm > bignum {
			impl.Dlascl(blas.General, 0, 0, bignum, anrm, 1, minmn-1, work[1:], minmn)
		}
		if anrm < smlnum {
			impl.Dlascl(blas.General, 0, 0, smlnum, anrm, 1, minmn, s, minmn)
		}
		if !ok && anrm < smlnum {
			impl.Dlascl(blas.General, 0, 0, smlnum, anrm, 1, minmn-1, work[1:], minmn)
		}
	}
	work[0] = float64(maxwrk)
	return ok
