...
// converged.
func (impl Implementation) Dgeev(jobvl blas.LeftEVJob, jobvr blas.RightEVJob, n int, a []float64, lda int, wr, wi []float64, vl []float64, ldvl int, vr []float64, ldvr int, work []float64, lwork int) (first int) {
	wantvl := jobvl == blas.LeftEVCompute
	wantvr := jobvr == blas.RightEVCompute
	var minwrk int
	if wantvl || wantvr {
		minwrk = max(1, 4*n)
	} else {
	...
	switch {
	case jobvl != blas.LeftEVCompute && jobvl != blas.LeftEVNone:
		panic(badLeftEVJob)
	case jobvr != blas.RightEVCompute && jobvr != blas.RightEVNone:
		panic(badRightEVJob)
	case n < 0:
		panic(nLT0)
	case lda < max(1, n):
	...
	if wantvl || wantvr {
		...
		impl.Dhseqr(blas.EigenvaluesAndSchur, blas.SchurOrig, n, 0, n-1,
		...
		impl.Dtrevc3(side, blas.EVAllMulQ, nil, n, a, lda, vl, ldvl, vr, ldvr,
			n, work, -1)
		maxwrk = max(maxwrk, n+int(work[0]))
		maxwrk = max(maxwrk, 4*n)
	} else {
		impl.Dhseqr(blas.EigenvaluesOnly, blas.SchurNone, n, 0, n-1,
	...
	anrm := impl.Dlange(blas.MaxAbs, n, n, a, lda, nil)
	var scalea bool
	var cscale float64
	...
	if scalea {
		impl.Dlascl(blas.General, 0, 0, anrm, cscale, n, n, a, lda)
	}

	// Balance the matrix.
	workbal := work[:n]
	ilo, ihi := impl.Dgebal(blas.PermuteScale, n, a, lda, workbal)
	...
	if wantvl {
		...
		iwrk = n
		first = impl.Dhseqr(blas.EigenvaluesAndSchur, blas.SchurOrig, n, ilo, ihi,
		...
		iwrk = n
		first = impl.Dhseqr(blas.EigenvaluesOnly, blas.SchurNone, n, ilo, ihi,
	...
	if first > 0 {
		if scalea {
			// Undo scaling.
			impl.Dlascl(blas.General, 0, 0, cscale, anrm, n-first, 1, wr[first:], 1)
			impl.Dlascl(blas.General, 0, 0, cscale, anrm, n-first, 1, wi[first:], 1)
			impl.Dlascl(blas.General, 0, 0, cscale, anrm, ilo, 1, wr, 1)
			impl.Dlascl(blas.General, 0, 0, cscale, anrm, ilo, 1, wi, 1)
		}
		work[0] = float64(maxwrk)
		return first
	}
	...
	if wantvl || wantvr {
		...
		impl.Dtrevc3(side, blas.EVAllMulQ, nil, n,
			a, lda, vl, ldvl, vr, ldvr, n, work[iwrk:], lwork-iwrk)
	}
	bi := blas64.Implementation()
	if wantvl {
		// Undo balancing of left eigenvectors.
		impl.Dgebak(blas.PermuteScale, blas.EVLeft, n, ilo, ihi, workbal, n, vl, ldvl)
	...
	if wantvr {
		...
		impl.Dgebak(blas.PermuteScale, blas.EVRight, n, ilo, ihi, workbal, n, vr, ldvr)
