...
// in t.
//
//	H = I - V * T * Vᵀ  if store == blas.ColumnWise
//	H = I - Vᵀ * T * V  if store == blas.RowWise
//
// H is defined by a product of the elementary reflectors where
//
//	H = H_0 * H_1 * ... * H_{k-1}  if direct == blas.Forward
//	H = H_{k-1} * ... * H_1 * H_0  if direct == blas.Backward
//
// t is a k×k triangular matrix. t is upper triangular if direct = blas.Forward
...
func (Implementation) Dlarft(direct blas.Direct, store blas.StoreV, n, k int, v []float64, ldv int, tau []float64, t []float64, ldt int) {
	mv, nv := n, k
	if store == blas.RowWise {
		mv, nv = k, n
	}
	switch {
	case direct != blas.Forward && direct != blas.Backward:
		panic(badDirect)
	case store != blas.RowWise && store != blas.ColumnWise:
		panic(badStoreV)
	case n < 0:
		panic(nLT0)
	case k < 1:
