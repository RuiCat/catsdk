...
func (impl Implementation) Dbdsqr(uplo blas.Uplo, n, ncvt, nru, ncc int, d, e, vt []float64, ldvt int, u []float64, ldu int, c []float64, ldc int, work []float64) (ok bool) {
	...
	if n != 1 {
		...
		if lower {
			...
			if nru > 0 {
				impl.Dlasr(blas.Right, blas.Variable, blas.Forward, nru, n, work, work[n-1:], u, ldu)
			}
			if ncc > 0 {
				impl.Dlasr(blas.Left, blas.Variable, blas.Forward, n, ncc, work, work[n-1:], c, ldc)
			}
		}
		...
		for m > 1 {
			...
			if shift == 0 {
				if idir == 1 {
					...
					e[m-2] = h * oldsn
					if ncvt > 0 {
						impl.Dlasr(blas.Left, blas.Variable, blas.Forward, m-l2, ncvt, work, work[n-1:], vt[l2*ldvt:], ldvt)
					}
					if nru > 0 {
						impl.Dlasr(blas.Right, blas.Variable, blas.Forward, nru, m-l2, work[nm12:], work[nm13:], u[l2:], ldu)
					}
					if ncc > 0 {
						impl.Dlasr(blas.Left, blas.Variable, blas.Forward, m-l2, ncc, work[nm12:], work[nm13:], c[l2*ldc:], ldc)
					}
					...
					e[l2] = h * oldsn
					if ncvt > 0 {
						impl.Dlasr(blas.Left, blas.Variable, blas.Backward, m-l2, ncvt, work[nm12:], work[nm13:], vt[l2*ldvt:], ldvt)
					}
					if nru > 0 {
						impl.Dlasr(blas.Right, blas.Variable, blas.Backward, nru, m-l2, work, work[n-1:], u[l2:], ldu)
					}
					if ncc > 0 {
						impl.Dlasr(blas.Left, blas.Variable, blas.Backward, m-l2, ncc, work, work[n-1:], c[l2*ldc:], ldc)
					}
