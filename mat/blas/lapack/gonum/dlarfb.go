...
//
// If direct == blas.Forward and store == blas.ColumnWise
//
//	V = [ 1        ]
//	    [v1   1    ]
//	    [v1  v2   1]
...
//	    [v1  v2  v3]
//
// If direct == blas.Forward and store == blas.RowWise
//
//	V = [ 1  v1  v1  v1  v1]
//	    [     1  v2  v2  v2]
//	    [         1  v3  v3]
//
// If direct == blas.Backward and store == blas.ColumnWise
//
//	V = [v1  v2  v3]
//	    [v1  v2  v3]
//	    [ 1  v2  v3]
...
//	    [         1]
//
// If direct == blas.Backward and store == blas.RowWise
//
//	V = [v1  v1   1        ]
//	    [v2  v2  v2   1    ]
//	    [v3  v3  v3  v3   1]
...
func (Implementation) Dlarfb(side blas.Side, trans blas.Transpose, direct blas.Direct, store blas.StoreV, m, n, k int, v []float64, ldv int, t []float64, ldt int, c []float64, ldc int, work []float64, ldwork int) {
	nv := m
	if side == blas.Right {
		nv = n
	}
	...
	case direct != blas.Forward && direct != blas.Backward:
		panic(badDirect)
	case store != blas.ColumnWise && store != blas.RowWise:
		panic(badStoreV)
	case m < 0:
		panic(mLT0)
	case n < 0:
	...
	case k < 0:
		panic(kLT0)
	case store == blas.ColumnWise && ldv < max(1, k):
		panic(badLdV)
	case store == blas.RowWise && ldv < max(1, nv):
		panic(badLdV)
	case ldt < max(1, k):
		panic(badLdT)
	case ldc < max(1, n):
	...
	switch {
	case store == blas.ColumnWise && len(v) < (nv-1)*ldv+k:
		panic(shortV)
	case store == blas.RowWise && len(v) < (k-1)*ldv+nv:
		panic(shortV)
