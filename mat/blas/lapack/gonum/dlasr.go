...
// by a depending on the value of side -- A = P * A if side == blas.Left,
// A = A * Pᵀ if side == blas.Right.
//
...
//	       [-s[k] s[k]]
//
// If direct == blas.Forward, the rotation matrices are applied as
// P = P(z-1) * ... * P(2) * P(1), while if direct == blas.Backward they are
// applied as P = P(1) * P(2) * ... * P(n).
//
// pivot defines the mapping of the elements in R(k) to P(k).
// If pivot == blas.Variable, the rotation is performed for the (k, k+1) plane.
//
//	P(k) = [1                    ]
//	       [    ...              ]
//	       [     1               ]
...
//	       [                    1]
//
// if pivot == blas.Top, the rotation is performed for the (1, k+1) plane,
//
//	P(k) = [c[k]        s[k]     ]
//	       [    1                ]
//	       [     ...             ]
...
//	       [                    1]
//
// and if pivot == blas.Bottom, the rotation is performed for the (k, z) plane.
//
//	P(k) = [1                    ]
//	       [  ...                ]
//	       [      1              ]
...
func (impl Implementation) Dlasr(side blas.Side, pivot blas.Pivot, direct blas.Direct, m, n int, c, s, a []float64, lda int) {
	switch {
	case side != blas.Left && side != blas.Right:
		panic(badSide)
	case pivot != blas.Variable && pivot != blas.Top && pivot != blas.Bottom:
		panic(badPivot)
	case direct != blas.Forward && direct != blas.Backward:
		panic(badDirect)
	case m < 0:
		panic(mLT0)
	case n < 0:
	...
	if side == blas.Left {
		if pivot == blas.Variable {
			if direct == blas.Forward {
				for j := 0; j < m-1; j++ {
					ctmp := c[j]
					stmp := s[j]
					if ctmp != 1 || stmp != 0 {
