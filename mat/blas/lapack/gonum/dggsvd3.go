...
func (impl Implementation) Dggsvd3(jobU, jobV, jobQ blas.GSVDJob, m, n, p int, a []float64, lda int, b []float64, ldb int, alpha, beta, u []float64, ldu int, v []float64, ldv int, q []float64, ldq int, work []float64, lwork int, iwork []int) (k, l int, ok bool) {
	wantu := jobU == blas.GSVDU
	wantv := jobV == blas.GSVDV
	wantq := jobQ == blas.GSVDQ
	switch {
	case !wantu && jobU != blas.GSVDNone:
		panic(badGSVDJob + "U")
	case !wantv && jobV != blas.GSVDNone:
		panic(badGSVDJob + "V")
	case !wantq && jobQ != blas.GSVDNone:
		panic(badGSVDJob + "Q")
	case m < 0:
		panic(mLT0)
	case n < 0:
	...
	anorm := impl.Dlange(blas.Frobenius, m, n, a, lda, nil)
	bnorm := impl.Dlange(blas.Frobenius, p, n, b, ldb, nil)
