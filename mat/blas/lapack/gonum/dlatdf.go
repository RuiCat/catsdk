...
//
// If job is blas.LocalLookAhead, all entries of h are chosen as either +1 or
// -1.
//
// If job is blas.NormalizedNullVector, an approximate null-vector e of Z is
...
func (impl Implementation) Dlatdf(job blas.MaximizeNormXJob, n int, z []float64, ldz int, rhs []float64, rdsum, rdscal float64, ipiv, jpiv []int) (scale, sum float64) {
	switch {
	case job != blas.LocalLookAhead && job != blas.NormalizedNullVector:
	...
	xp := xps[:n]
	xm := xms[:n]
	if job == blas.NormalizedNullVector {
		// Compute approximate nullvector xm of Z.
		_ = impl.Dgecon(blas.MaxRowSum, n, z, ldz, 1, work[:], iwork[:])
