...
// tridiagonal form if compz == blas.EVOrig. On exit, if
// compz == blas.EVOrig, z contains the orthonormal eigenvectors of the
// original symmetric matrix, and if compz == blas.EVTridiag, z contains the
// orthonormal eigenvectors of the symmetric tridiagonal matrix. z is not used
// if compz == blas.EVCompNone.
//
...
func (impl Implementation) Dsteqr(compz blas.EVComp, n int, d, e, z []float64, ldz int, work []float64) (ok bool) {
	switch {
	case compz != blas.EVCompNone && compz != blas.EVTridiag && compz != blas.EVOrig:
		panic(badEVComp)
	case n < 0:
		panic(nLT0)
	case ldz < 1, compz != blas.EVCompNone && ldz < n:
		panic(badLdZ)
	}
	...
	case len(e) < n-1:
		panic(shortE)
	case compz != blas.EVCompNone && len(z) < (n-1)*ldz+n:
		panic(shortZ)
	case compz != blas.EVCompNone && len(work) < max(1, 2*n-2):
		panic(shortWork)
	}

	var icompz int
	if compz == blas.EVOrig {
		icompz = 1
	} else if compz == blas.EVTridiag {
		icompz = 2
	}
	...
	for {
		if l1 > n-1 {
			...
			if icompz == 0 {
				impl.Dlasrt(blas.SortIncreasing, n, d)
			} else {
		...
		anorm := impl.Dlanst(blas.MaxAbs, lend-l+1, d[l:], e[l:])
		switch {
		case anorm == 0:
			continue
		case anorm > ssfmax:
			...
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmax, lend-l+1, 1, d[l:], 1)
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmax, lend-l, 1, e[l:], 1)
		case anorm < ssfmin:
			iscale = up
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmin, lend-l+1, 1, d[l:], 1)
			impl.Dlascl(blas.General, 0, 0, anorm, ssfmin, lend-l, 1, e[l:], 1)
		}
			...
			for {
				...
				if m == l+1 {
					if icompz > 0 {
						...
						impl.Dlasr(blas.Right, blas.Variable, blas.Backward,
			...
			for {
				...
				if m == l-1 {
					if icompz > 0 {
						...
						impl.Dlasr(blas.Right, blas.Variable, blas.Forward,
		...
		case down:
			// Pretend that d and e are matrices with 1 column.
			impl.Dlascl(blas.General, 0, 0, ssfmax, anorm, lendsv-lsv+1, 1, d[lsv:], 1)
			impl.Dlascl(blas.General, 0, 0, ssfmax, anorm, lendsv-lsv, 1, e[lsv:], 1)
		case up:
			impl.Dlascl(blas.General, 0, 0, ssfmin, anorm, lendsv-lsv+1, 1, d[lsv:], 1)
			impl.Dlascl(blas.General, 0, 0, ssfmin, anorm, lendsv-lsv, 1, e[lsv:], 1)
		}
