...
// Q and Pᵀ.
//
// If vect == blas.GenerateQ, then a is assumed to have been an m×k matrix and
...
//
// If vect == blas.GeneratePT, then A is assumed to have been a k×n matrix, and
...
func (impl Implementation) Dorgbr(vect blas.GenOrtho, m, n, k int, a []float64, lda int, tau, work []float64, lwork int) {
	wantq := vect == blas.GenerateQ
	mn := min(m, n)
	switch {
	case vect != blas.GenerateQ && vect != blas.GeneratePT:
		panic(badGenOrtho)
	case m < 0:
		panic(mLT0)
	case n < 0:
