...
//
// If compq is blas.UpdateSchur, on return the matrix Q of Schur vectors will be
// updated by post-multiplying it with Z.
// If compq is blas.UpdateSchurNone, the matrix Q is not referenced and will not be
// updated.
...
func (impl Implementation) Dtrexc(compq blas.UpdateSchurComp, n int, t []float64, ldt int, q []float64, ldq int, ifst, ilst int, work []float64) (ifstOut, ilstOut int, ok bool) {
	switch {
	case compq != blas.UpdateSchur && compq != blas.UpdateSchurNone:
		panic(badUpdateSchurComp)
	case n < 0:
		panic(nLT0)
	case ldt < max(1, n):
		panic(badLdT)
	case ldq < 1, compq == blas.UpdateSchur && ldq < n:
		panic(badLdQ)
	...
	case len(t) < (n-1)*ldt+n:
		panic(shortT)
	case compq == blas.UpdateSchur && len(q) < (n-1)*ldq+n:
		panic(shortQ)
	case len(work) < n:
		panic(shortWork)
	}
