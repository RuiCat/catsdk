...
func (impl Implementation) Dsyev(jobz blas.EVJob, uplo blas.Uplo, n int, a []float64, lda int, w, work []float64, lwork int) (ok bool) {
	switch {
	case jobz != blas.EVNone && jobz != blas.EVCompute:
		panic(badEVJob)
	...
	anrm := impl.Dlansy(blas.MaxAbs, uplo, n, a, lda, work)
	scaled := false
	var sigma float64
	if anrm > 0 && anrm < rmin {
		scaled = true
	...
	if jobz == blas.EVNone {
		...
		ok = impl.Dsteqr(blas.EVComp(jobz), n, w, work[inde:], a, lda, work[indtau:])
	}
	if !ok {
		return false
	}
