...
//
// If side == blas.EVRight, only right eigenvectors will be computed.
// If side == blas.EVLeft, only left eigenvectors will be computed.
// If side == blas.EVBoth, both right and left eigenvectors will be computed.
// For other values of side, Dtrevc3 will panic.
//
// If howmny == blas.EVAll, all right and/or left eigenvectors will be
// computed.
// If howmny == blas.EVAllMulQ, all right and/or left eigenvectors will be
// computed and multiplied from left by the matrices in VR and/or VL.
// If howmny == blas.EVSelected, right and/or left eigenvectors will be
...
// if howmny == blas.EVSelected, and it is not referenced otherwise.
...
//
// VL and VR are n×mm matrices. If howmny is blas.EVAll or
// blas.AllEVMulQ, mm must be at least n. If howmny is
// blas.EVSelected, mm must be large enough to store the selected
...
//
// On entry, if howmny is blas.EVAllMulQ, it is assumed that VL (if side
// is blas.EVLeft or blas.EVBoth) contains an n×n matrix QL,
// and that VR (if side is blas.EVRight or blas.EVBoth) contains
// an n×n matrix QR. QL and QR are typically the orthogonal matrix Q of Schur
// vectors returned by Dhseqr.
//
// On return, if side is blas.EVLeft or blas.EVBoth,
// VL will contain:
//
//	if howmny == blas.EVAll,      the matrix Y of left eigenvectors of T,
//	if howmny == blas.EVAllMulQ,  the matrix Q*Y,
//	if howmny == blas.EVSelected, the left eigenvectors of T specified by
//	                                selected, stored consecutively in the
//	                                columns of VL, in the same order as their
//	                                eigenvalues.
//
// VL is not referenced if side == blas.EVRight.
//
// On return, if side is blas.EVRight or blas.EVBoth,
// VR will contain:
//
//	if howmny == blas.EVAll,      the matrix X of right eigenvectors of T,
//	if howmny == blas.EVAllMulQ,  the matrix Q*X,
//	if howmny == blas.EVSelected, the left eigenvectors of T specified by
//	                                selected, stored consecutively in the
//	                                columns of VR, in the same order as their
//	                                eigenvalues.
//
// VR is not referenced if side == blas.EVLeft.
//
...
func (impl Implementation) Dtrevc3(side blas.EVSide, howmny blas.EVHowMany, selected []bool, n int, t []float64, ldt int, vl []float64, ldvl int, vr []float64, ldvr int, mm int, work []float64, lwork int) (m int) {
	bothv := side == blas.EVBoth
	rightv := side == blas.EVRight || bothv
	leftv := side == blas.EVLeft || bothv
	switch {
	case !rightv && !leftv:
		panic(badEVSide)
	case howmny != blas.EVAll && howmny != blas.EVAllMulQ && howmny != blas.EVSelected:
		panic(badEVHowMany)
	case n < 0:
		panic(nLT0)
	case ldt < max(1, n):
	...
	if howmny == blas.EVAllMulQ && lwork >= n+2*n*nbmin {
...
leftev:
	...
	for ki := 0; ki < n; ki++ {
		...
		if howmny == blas.EVSelected && !selected[ki] {
			continue
		}
