...
// If job is blas.BalanceNone, Dgebal sets scale[i] = 1 for all i and returns ilo=0, ihi=n-1.
// If job is blas.Permute, only permuting will be done.
// If job is blas.Scale, only scaling will be done.
// If job is blas.PermuteScale, both permuting and scaling will be done.
//
// On return, if job is blas.Permute or blas.PermuteScale, it will hold that
//
//	A[i,j] == 0,   for i > j and j ∈ {0, ..., ilo-1, ihi+1, ..., n-1}.
//
// If job is blas.BalanceNone or blas.Scale, or if n == 0, it will hold that
//
//	ilo == 0 and ihi == n-1.
//
...
func (impl Implementation) Dgebal(job blas.BalanceJob, n int, a []float64, lda int, scale []float64) (ilo, ihi int) {
	switch {
