...
//	A = Q H Qᵀ = (QZ) T (QZ)ᵀ.
//
// If job == blas.EigenvaluesOnly, only the eigenvalues will be computed.
// If job == blas.EigenvaluesAndSchur, the eigenvalues and the Schur form T will
// be computed.
// For other values of job Dhseqr will panic.
//
// If compz == blas.SchurNone, no Schur vectors will be computed and Z will not be
// referenced.
// If compz == blas.SchurHess, on return Z will contain the matrix of Schur
// vectors of H.
// If compz == blas.SchurOrig, on entry z is assumed to contain the orthogonal
...
// and wi[i+1] < 0.
//
// If unconverged == 0 and job == blas.EigenvaluesAndSchur, on return H will
...
//	wi[i+1] = -wi[i].
//
// If unconverged == 0 and job == blas.EigenvaluesOnly, the contents of h
...
//
// If unconverged > 0 and compz == blas.SchurOrig, then on return
//
//	(final Z) = (initial Z) U,
//
// where U is the orthogonal matrix in (*) regardless of the value of job.
//
// If unconverged > 0 and compz == blas.SchurHess, then on return
//
//	(final Z) = U,
//
// where U is the orthogonal matrix in (*) regardless of the value of job.
...
func (impl Implementation) Dhseqr(job blas.SchurJob, compz blas.SchurComp, n, ilo, ihi int, h []float64, ldh int, wr, wi []float64, z []float64, ldz int, work []float64, lwork int) (unconverged int) {
	wantt := job == blas.EigenvaluesAndSchur
	wantz := compz == blas.SchurHess || compz == blas.SchurOrig

	switch {
	case job != blas.EigenvaluesOnly && job != blas.EigenvaluesAndSchur:
		panic(badSchurJob)
	case compz != blas.SchurNone && compz != blas.SchurHess && compz != blas.SchurOrig:
		panic(badSchurComp)
	case n < 0:
		panic(nLT0)
