...
func calcJumpToAbsAddr(_ int, addr uintptr, code []byte) uintptr {
	sz := 0
	offset := 0
...
func doFixTargetFuncCode(all bool, mode int, start uintptr, funcSz int, to uintptr, move_sz int, inst []CodeFix) ([]CodeFix, error) {
	...
	for i = 0; i < len(inst); i++ {
		...
		if sz == 0 && ft == FT_INVALID {
			...
			return nil, fmt.Errorf("invalid instruction scanned, addr:0x%x", curAddr)
		...
		if ft == FT_RET {
			return nil, fmt.Errorf("ret instruction in patching erea is not allowed, addr:0x%x", curAddr)
		}

		if ft == FT_OVERFLOW {
			return nil, fmt.Errorf("jmp instruction in patching erea overflow, addr:0x%x", curAddr)
		}
	...
	for ; i < len(inst); i++ {
		...
		if ft == FT_OVERFLOW {
			return nil, fmt.Errorf("jmp instruction in body overflow, addr:0x%x", curAddr)
		}
...
func GetFuncSizeByGuess(mode int, start uintptr, minimal bool) (uint32, error) {
	...
	if !bytes.Equal(funcPrologue, code[:prologueLen]) { // not valid function start or invalid prologue
		return 0, fmt.Errorf("no func prologue, addr:0x%x", start))
	}
	*/
