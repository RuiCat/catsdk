...
func Play(sampleRate int, channelCount int, readFunc func(buf []float32), bufferSizeInBytes int) error {
	...
	if msg := C.player_oboe_Play(C.int(sampleRate), C.int(channelCount), C.int(bufferSizeInBytes)); msg != nil {
...
func Suspend() error {
	if msg := C.player_oboe_Suspend(); msg != nil {
...
func Resume() error {
	if msg := C.player_oboe_Resume(); msg != nil {
		return fmt.Errorf("oboe: Resume failed: %s", C.GoString(msg))
	}
	return nil
}

//export player_oboe_read
func player_oboe_read(buf *C.float, len C.size_t) {
