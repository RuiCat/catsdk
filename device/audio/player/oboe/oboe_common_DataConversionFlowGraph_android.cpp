...
#include "oboe_flowgraph_MonplayerMultiConverter_android.h"
...
Result DataConversionFlowGraph::configure(AudioStream *sourceStream, AudioStream *sinkStream) {
    ...
    if (sourceChannelCount < sinkChannelCount) {
        if (sourceChannelCount == 1) {
            mMonplayerMultiConverter = std::make_unique<MonplayerMultiConverter>(sinkChannelCount);
            lastOutput->connect(&mMonplayerMultiConverter->input);
            lastOutput = &mMonplayerMultiConverter->output;
        } else {
            mChannelCountConverter = std::make_unique<ChannelCountConverter>(
                    sourceChannelCount,
                    sinkChannelCount);
