...
void Stream::Loop(int num_frames) {
  ...
  for (;;) {
    ...
    player_oboe_read(&tmp[0], tmp.size());
    {
...
const char *player_oboe_Play(int sample_rate, int channel_num,
                          int buffer_size_in_bytes) {
  return Stream::GetInstance().Play(sample_rate, channel_num,
                                    buffer_size_in_bytes);
}

const char *player_oboe_Suspend() { return Stream::GetInstance().Pause(); }

const char *player_oboe_Resume() { return Stream::GetInstance().Resume(); }
