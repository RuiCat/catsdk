 ...
 */
class MonplayerMultiConverter : public FlowGraphNode {
public:
    explicit MonplayerMultiConverter(int32_t outputChannelCount);

    virtual ~MonplayerMultiConverter() = default;

    int32_t onProcess(int32_t numFrames) override;

    const char *getName() override {
        return "MonplayerMultiConverter";
    }
