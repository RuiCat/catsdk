...
func (f *freelist) copyallunsafe(dstptr unsafe.Pointer) { // dstptr is []pgid data pointer
	...
	sz := len(fpgids) + len(m)
	dst := unsafe.Slice((*pgid)(unsafe.Pointer(uintptr(dstptr))), int(sz))
	/*
...
func (f *freelist) read(p *page) {
	...
	if count == 0 {
		...
		ids := unsafe.Slice((*pgid)(unsafe.Pointer(uintptr(unsafe.Pointer(p))+unsafe.Sizeof(*p)+idx*unsafe.Sizeof(pgid(0)))), int(count))
		/*
