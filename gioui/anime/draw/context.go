// package draw provides a simple API for rendering 2D graphics in pure Go.
package draw
...
var quantizeMedianCutQuantizer = quantize.MedianCutQuantizer{AddTransparent: true}

// Paletted 得到调色板
func (dc *Context) Paletted() *image.Paletted {
	bounds := dc.im.Bounds()
	palettedImage := image.NewPaletted(bounds, quantizeMedianCutQuantizer.Quantize(make([]color.Color, 0, 256), dc.im))
	draw.Src.Draw(palettedImage, bounds, dc.im, bounds.Min)
	return palettedImage
}

// ClearPix 清除绘制
func (dc *Context) ClearPix() {
	clear(dc.im.Pix)
	dc.ClearPath()
}
