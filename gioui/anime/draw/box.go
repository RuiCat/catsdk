...
// Box 盒子结构和操作函数
type Box struct {
	Min, Max Vector // 3D盒子的最小和最大点坐标
}

// BoxINF 的Max是正无穷
var BoxINF = Box{Min: Vector{X: -INF, Y: -INF, Z: -INF}, Max: Vector{X: INF, Y: INF, Z: INF}}

// BoxForShapes 计算给定形状组的外接盒子
...
func (a Box) Center() Vector {
	return a.Anchor(Vector{X: 0.5, Y: 0.5, Z: 0.5})
}

// OuterRadius 计算给定盒子外接圆半径
func (a Box) OuterRadius() float64 {
	return a.Min.Sub(a.Center()).Length()
}

// InnerRadius 计算给定盒子内接圆半径
...
func (b *Box) Intersect(r Ray) (float64, float64) {
	...
	t1 := math.Max(math.Max(x1, y1), z1) // 最小交点
	t2 := math.Min(math.Min(x2, y2), z2) // 最大交点
	return t1, t2
}

// Partition 判断给定盒子在指定轴上的点是否在左/右半区
