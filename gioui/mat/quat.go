...
func QuatIdent[T Float]() Quat[T] {
	return Quat[T]{1., Vec3[T]{0, 0, 0}}
}

// QuatRotate creates an angle from an axis and an angle relative to that axis.
//
// This is cheaper than HomogRotate3D.
func QuatRotate[T Float](angle float64, axis Vec3[T]) Quat[T] {
	// angle = (float32(math.Pi) * angle) / 180.0

	c, s := T(math.Cos(float64(angle/2))), T(math.Sin(float64(angle/2)))

	return Quat[T]{c, axis.Mul(s)}
}
...
func (q1 Quat[T]) Add(q2 Quat[T]) Quat[T] {
	return Quat[T]{q1.W + q2.W, q1.V.Add(q2.V)}
}

// Sub subtracts two quaternions. It's no more complicated than
// subtracting their W and V components.
func (q1 Quat[T]) Sub(q2 Quat[T]) Quat[T] {
	return Quat[T]{q1.W - q2.W, q1.V.Sub(q2.V)}
}
...
// equal q2.Mul(q1).
func (q1 Quat[T]) Mul(q2 Quat[T]) Quat[T] {
	return Quat[T]{q1.W*q2.W - q1.V.Dot(q2.V), q1.V.Cross(q2.V).Add(q2.V.Mul(q1.W)).Add(q1.V.Mul(q2.W))}
}

// Scale every element of the quaternion by some constant factor.
func (q1 Quat[T]) Scale(c T) Quat[T] {
	return Quat[T]{q1.W * c, Vec3[T]{q1.V[0] * c, q1.V[1] * c, q1.V[2] * c}}
}
 
// Conjugate returns the conjugate of a quaternion. Equivalent to
// Quat[T]{q1.W, q1.V.Mul(-1)}.
func (q1 Quat[T]) Conjugate() Quat[T] {
	return Quat[T]{q1.W, q1.V.Mul(-1)}
}

// Len gives the Length of the quaternion, also known as its Norm. This is the
// same thing as the Len of a Vec4.
func (q1 Quat[T]) Len() T {
	return T(math.Sqrt(float64(q1.W*q1.W + q1.V[0]*q1.V[0] + q1.V[1]*q1.V[1] + q1.V[2]*q1.V[2])))
}

// Norm is an alias for Len() since both are very common terms.
func (q1 Quat[T]) Norm() T {
	return q1.Len()
}
...
func (q1 Quat[T]) Normalize() Quat[T] {
	length := q1.Len()
	...
	return Quat[T]{T(q1.W * 1 / length), q1.V.Mul(T(1 / length))}
}
...
// q1.Mul(Quat[T]{0,v}).Mul(q1.Conjugate()), and
...
func (q1 Quat[T]) Rotate(v Vec3[T]) Vec3[T] {
	cross := q1.V.Cross(v)
...
func (q1 Quat[T]) Dot(q2 Quat[T]) T {
...
func (q1 Quat[T]) ApproxEqual(q2 Quat[T]) bool {
...
func (q1 Quat[T]) ApproxEqualThreshold(q2 Quat[T], epsilon T) bool {
...
func (q1 Quat[T]) ApproxEqualFunc(q2 Quat[T], f func(T, T) bool) bool {
...
func (q1 Quat[T]) OrientationEqual(q2 Quat[T]) bool {
	return q1.OrientationEqualThreshold(q2, T(Epsilon))
}

// OrientationEqualThreshold returns whether the quaternions represents the same orientation with a given tolerence
func (q1 Quat[T]) OrientationEqualThreshold(q2 Quat[T], epsilon T) bool {
...
func QuatSlerp[T Float](q1, q2 Quat[T], amount T) Quat[T] {
	...
	theta := T(math.Acos(float64(dot))) * amount
	c, s := T(math.Cos(float64(theta))), T(math.Sin(float64(theta)))
...
func QuatLerp[T Float](q1, q2 Quat[T], amount T) Quat[T] {
...
func QuatNlerp[T Float](q1, q2 Quat[T], amount T) Quat[T] {
	return QuatLerp[T](q1, q2, amount).Normalize()
}
...
func AnglesToQuat[T Float](angle1, angle2, angle3 T, order RotationOrder) Quat[T] {
	var s [3]float64
	var c [3]float64
	...
	switch order {
	case ZYX:
		ret.W = T(c[0]*c[1]*c[2] + s[0]*s[1]*s[2])
		ret.V = Vec3[T]{T(c[0]*c[1]*s[2] - s[0]*s[1]*c[2]),
			T(c[0]*s[1]*c[2] + s[0]*c[1]*s[2]),
			T(s[0]*c[1]*c[2] - c[0]*s[1]*s[2]),
		}
	case ZYZ:
		ret.W = T(c[0]*c[1]*c[2] - s[0]*c[1]*s[2])
		ret.V = Vec3[T]{T(c[0]*s[1]*s[2] - s[0]*s[1]*c[2]),
			T(c[0]*s[1]*c[2] + s[0]*s[1]*s[2]),
			T(s[0]*c[1]*c[2] + c[0]*c[1]*s[2]),
		}
	case ZXY:
		ret.W = T(c[0]*c[1]*c[2] - s[0]*s[1]*s[2])
		ret.V = Vec3[T]{T(c[0]*s[1]*c[2] - s[0]*c[1]*s[2]),
			T(c[0]*c[1]*s[2] + s[0]*s[1]*c[2]),
			T(c[0]*s[1]*s[2] + s[0]*c[1]*c[2]),
		}
	case ZXZ:
		ret.W = T(c[0]*c[1]*c[2] - s[0]*c[1]*s[2])
		ret.V = Vec3[T]{T(c[0]*s[1]*c[2] + s[0]*s[1]*s[2]),
			T(s[0]*s[1]*c[2] - c[0]*s[1]*s[2]),
			T(c[0]*c[1]*s[2] + s[0]*c[1]*c[2]),
		}
	case YXZ:
		ret.W = T(c[0]*c[1]*c[2] + s[0]*s[1]*s[2])
		ret.V = Vec3[T]{T(c[0]*s[1]*c[2] + s[0]*c[1]*s[2]),
	...
	case YZY:
		ret.W = T(c[0]*c[1]*c[2] - s[0]*c[1]*s[2])
		ret.V = Vec3[T]{T(s[0]*s[1]*c[2] - c[0]*s[1]*s[2]),
			T(c[0]*c[1]*s[2] + s[0]*c[1]*c[2]),
			T(c[0]*s[1]*c[2] + s[0]*s[1]*s[2]),
		}
	case XYZ:
		ret.W = T(c[0]*c[1]*c[2] - s[0]*s[1]*s[2])
		ret.V = Vec3[T]{T(c[0]*s[1]*s[2] + s[0]*c[1]*c[2]),
			T(c[0]*s[1]*c[2] - s[0]*c[1]*s[2]),
			T(c[0]*c[1]*s[2] + s[0]*s[1]*c[2]),
		}
	case XYX:
		ret.W = T(c[0]*c[1]*c[2] - s[0]*c[1]*s[2])
		ret.V = Vec3[T]{T(c[0]*c[1]*s[2] + s[0]*c[1]*c[2]),
			T(c[0]*s[1]*c[2] + s[0]*s[1]*s[2]),
			T(s[0]*s[1]*c[2] - c[0]*s[1]*s[2]),
		}
	case XZY:
		ret.W = T(c[0]*c[1]*c[2] + s[0]*s[1]*s[2])
		ret.V = Vec3[T]{T(s[0]*c[1]*c[2] - c[0]*s[1]*s[2]),
			T(c[0]*c[1]*s[2] - s[0]*s[1]*c[2]),
			T(c[0]*s[1]*c[2] + s[0]*c[1]*s[2]),
		}
	case XZX:
...
func Mat4ToQuat[T Float](m Mat4[T]) Quat[T] {
	// http://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToQuaternion/index.htm

	if tr := m[0] + m[5] + m[10]; tr > 0 {
		s := T(0.5 / math.Sqrt(float64(tr+1.0)))
		return Quat[T]{
			W: T(0.25 / s),
			V: Vec3[T]{
				(m[6] - m[9]) * s,
	...
	if (m[0] > m[5]) && (m[0] > m[10]) {
		s := T(2.0 * math.Sqrt(float64(1.0+m[0]-m[5]-m[10])))
		return Quat[T]{
			W: (m[6] - m[9]) / s,
			V: Vec3[T]{
				0.25 * s,
	...
	if m[5] > m[10] {
		s := T(2.0 * math.Sqrt(float64(1.0+m[5]-m[0]-m[10])))
		return Quat[T]{
			W: (m[8] - m[2]) / s,
			V: Vec3[T]{
				(m[4] + m[1]) / s,
	...
	s := T(2.0 * math.Sqrt(float64(1.0+m[10]-m[0]-m[5])))
	return Quat[T]{
		W: (m[1] - m[4]) / s,
		V: Vec3[T]{
			(m[8] + m[2]) / s,
...
func QuatLookAtV[T Float](eye, center, up Vec3[T]) Quat[T] {
	...
	rotDir := QuatBetweenVectors(Vec3[T]{0, 0, -1}, direction)
	...
	upCur := rotDir.Rotate(Vec3[T]{0, 1, 0})
...
func QuatBetweenVectors[T Float](start, dest Vec3[T]) Quat[T] {
	// http://www.opengl-tutorial.org/intermediate-tutorials/tutorial-17-quaternions/#I_need_an_equivalent_of_gluLookAt__How_do_I_orient_an_object_towards_a_point__
	...
	if cosTheta < -1.0+epsilon {
		...
		axis := Vec3[T]{1, 0, 0}.Cross(start)
		if axis.Dot(axis) < epsilon {
			// bad luck, they were parallel, try again!
			axis = Vec3[T]{0, 1, 0}.Cross(start)
		}
	...
	s := T(math.Sqrt(float64(1.0+cosTheta) * 2.0))
