...
func FloatEqual[T Float](a, b T) bool {
	return FloatEqualThreshold(a, b, T(Epsilon))
}
...
// threshold.
func FloatEqualFunc[T Float](epsilon T) func(T, T) bool {
...
func FloatEqualThreshold[T Float](a, b, epsilon T) bool {
	if a == b { // Handles the case of inf or shortcuts the loop when no significant error has accumulated
		return true
	}

	diff := Abs(a - b)
	if a*b == 0 || diff < T(MinNormal) { // If a or b are 0 or both are extremely close to it
...
func Clamp[T Float](a, low, high T) T {
	if a < low {
		return low
	} else if a > high {
		return high
...
func ClampFunc[T Float](low, high T) func(T) T {
	return func(a T) T {
...
func IsClamped[T Float](a, low, high T) bool {
	return a >= low && a <= high
}

// SetMin sets a to b if a > b.
func SetMin[T Float](a, b *T) {
	if *b < *a {
		*a = *b
	}
}
...
func Round[T Float](v T, precision int) T {
	p := float64(precision)
	t := float64(v) * math.Pow(10, p)
	if t > 0 {
		return T(math.Floor(t+0.5) / math.Pow(10, p))
	}
	return T(math.Ceil(t-0.5) / math.Pow(10, p))
}
