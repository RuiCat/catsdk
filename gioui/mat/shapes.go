...
func Circle[T Float](radiusX, radiusY T, numSlices int) []Vec2[T] {
	twoPi := float64(2.0 * math.Pi)

	circlePoints := make([]Vec2[T], 0, numSlices*3)
	center := Vec2[T]{0.0, 0.0}
	previous := Vec2[T]{radiusX, 0.0}
	...
	for theta := twoPi / float64(numSlices); !FloatEqual(theta, twoPi); theta = Clamp(theta+twoPi/float64(numSlices), 0.0, twoPi) {
		...
		curr := Vec2[T]{T(cos) * radiusX, T(sin) * radiusY}
	...
	circlePoints = append(circlePoints, center, previous, Vec2[T]{radiusX, 0.0})
	return circlePoints
}
...
func RectVec2[T Float](width, height T) []Vec2[T] {
	return []Vec2[T]{
		{0.0, 0.0},
		{0.0, -height},
		{width, -height},
...
func QuadraticBezierCurve2D[T Float](t T, cPoint1, cPoint2, cPoint3 Vec2[T]) Vec2[T] {
	if t < 0.0 || t > 1.0 {
...
func QuadraticBezierCurve3D[T Float](t T, cPoint1, cPoint2, cPoint3 Vec3[T]) Vec3[T] {
	if t < 0.0 || t > 1.0 {
		panic("Can't interpolate on bezier curve with t out of range [0.0,1.0]")
	}
...
func CubicBezierCurve2D[T Float](t T, cPoint1, cPoint2, cPoint3, cPoint4 Vec2[T]) Vec2[T] {
	if t < 0.0 || t > 1.0 {
		panic("Can't interpolate on bezier curve with t out of range [0.0,1.0]")
	}
...
func CubicBezierCurve3D[T Float](t T, cPoint1, cPoint2, cPoint3, cPoint4 Vec3[T]) Vec3[T] {
	if t < 0.0 || t > 1.0 {
		panic("Can't interpolate on bezier curve with t out of range [0.0,1.0]")
	}
...
func BezierCurve2D[T Float](t T, cPoints []Vec2[T]) Vec2[T] {
	if t < 0.0 || t > 1.0 {
	...
	n := len(cPoints) - 1
	point := cPoints[0].Mul(T(math.Pow(float64(1.0-t), float64(n))))

	for i := 1; i <= n; i++ {
		point = point.Add(cPoints[i].Mul(T(float64(choose[float64](n, i)) * math.Pow(float64(1-t), float64(n-i)) * math.Pow(float64(t), float64(i))))) // P += P_i * nCi * (1-t)^(n-i) * t^i
	}
...
func BezierCurve3D[T Float](t T, cPoints []Vec3[T]) Vec3[T] {
	if t < 0.0 || t > 1.0 {
		panic("Input to bezier has t not in range [0,1]. If you think this is a precision error, use mathgl.Clamp[f|d] before calling this function")
	}
...
// curve.
func MakeBezierCurve2D[T Float](numPoints int, cPoints []Vec2[T]) (line []Vec2[T]) {
	line = make([]Vec2[T], numPoints)
	if numPoints == 0 {
		return
	} else if numPoints == 1 {
		line[0] = cPoints[0]
	...
	for i := 1; i < numPoints-1; i++ {
		line[i] = BezierCurve2D(Clamp(T(i)/T(numPoints-1), 0.0, 1.0), cPoints)
	}
...
func MakeBezierCurve3D[T Float](numPoints int, cPoints []Vec3[T]) (line []Vec3[T]) {
	line = make([]Vec3[T], numPoints)
	if numPoints == 0 {
		return
	} else if numPoints == 1 {
		line[0] = cPoints[0]
	...
	line[0] = cPoints[0]
	for i := 1; i < numPoints-1; i++ {
		line[i] = BezierCurve3D(Clamp(T(i)/T(numPoints-1), 0.0, 1.0), cPoints)
	}
	line[numPoints-1] = cPoints[len(cPoints)-1]
...
func BezierSurface[T Float](u, v T, cPoints [][]Vec3[T]) Vec3[T] {
	...
	point := cPoints[0][0].Mul(T(math.Pow(float64(1.0-u), float64(n)) * math.Pow(float64(1.0-v), float64(m))))
	...
	for i := 0; i <= n; i++ {
		for j := 0; j <= m; j++ {
			...
			point = point.Add(cPoints[i][j].Mul(T(float64(choose[float64](n, i)) * math.Pow(float64(u), float64(i)) * math.Pow(float64(1.0-u),float64(n-i)) * float64(choose[float64](m, j)) * math.Pow(float64(v), float64(j)) * math.Pow(float64(1.0-v), float64(m-j)))))
		}
	}
...
func BezierSplineInterpolate2D[T Float](t T, ranges [][2]T, cPoints [][]Vec2[T]) Vec2[T] {
...
//
// If t is out of the range of all given curves, this function will panic
func BezierSplineInterpolate3D[T Float](t T, ranges [][2]T, cPoints [][]Vec3[T]) Vec3[T] {
	if len(ranges) != len(cPoints) {
		panic("Each bezier curve needs a range")
	}
...
func ReticulateSplines[T Float](ranges [][][2]T, cPoints [][][]Vec2[T], withLlamas bool) {
	if !withLlamas {
...
func ScreenToGLCoords[T Float](x, y int, screenWidth, screenHeight int) (xOut, yOut float64) {
...
func GLToScreenCoords[T Float](x, y float64, screenWidth, screenHeight int) (xOut, yOut int) {
...
func choose[T Float](n, k int) int {
	if k == 0 {
		return 1
	} else if n == 0 {
		return 0
